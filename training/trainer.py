"""AMP training loop with time limits, validation warm-up, and checkpointing."""
...
from training.gpu_optimizer import empty_cache_periodically, log_memory_snapshot
...
def cosine_lr(step: int, learning_rate: float, warmup_steps: int, total_steps: int) -> float:
...
@torch.no_grad()
def evaluate(model, val_loader, device, config) -> tuple[float, float]:
    model.eval()
    losses = []
    max_batches = 50
...
def train(model, train_loader, val_loader, optimizer, device, config, resume_step: int = 0, scaler=None) -> None:
    ...
    pbar = tqdm(total=time_limit, unit="s", desc="Training")
        ...
        for x, y in cycle(train_loader):
            ...
            if step > 0 and step % val_every == 0:
                if step < val_warmup:
                    ...
                    log(f"Step {step}: val_loss={val_loss:.4f}, val_ppl={val_ppl:.2f}")
                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        checkpointing.save_best(model, optimizer, scaler, step, val_loss, config)
